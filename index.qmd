...
- This mixed-methods study explores the acceptability and feasibility of personal sensing methods in a national sample of patients with opioid use disorder. It examines overall feasibility related to adherence and retention for providing personally sensed data for up to twelve months. It also uses two complementary qualitative methods to assess participant feedback on their experiences with various sensing methods after using the methods for 3-4 months. Finally, this study specifically assesses differences in patient adherence and feedback by sensing method and three demographic characteristics (gender, race/ethnicity, and geographic location). 
...
For the active methods, participants rated how much they agreed with two statements using 5-point bipolar scales (strongly agree, agree, agree and disagree equally, disagree, or strongly disagree). The third item was an open-ended prompt. Specifically participants responded to:
...
We evaluated whether length of study participation differed across our 4 demographic subgroups (gender, race/ethnicity, income, and geographic location) to understand whether certain subgroups were better able and more willing to adhere to the study protocol. Specifically, we conducted a single linear model where length of participation (in days) was regressed on the four demographic grouping variables (centered around 0). We did not assess for subgroup differences in daily and monthly survey adherence rates to minimize the number of analyses. Participants were withdrawn from the study if adherence dropped, thus differences in adherence are likely captured in length of participation. 

### Participant Experience Questionannaire Quantitative Analyses
Participants rated how much they agreed with 7 statements about sensing method interference and dislike using a 5-point scale with descriptive anchors (strongly agree, agree, agree and disagree equally, disagree, or strongly disagree). We recoded these items to a numeric scale ranging from −2 to 2, with 0 representing the neutral (agree and disagree equally) midpoint and higher scores representing greater acceptability. We fit 7 linear models (one for each item) that regressed the acceptability rating on the four demographic grouping variables (centered around 0). We used these models to understand whether acceptability, on average, differed from the neutral midpoint (0/agree and disagree equally; intercept). We also evaluated each demographic grouping parameter to determine if differences in acceptability ratings existed among subgroups. We also calculated intraclass correlations (single, case 3 [@shroutIntraclassCorrelationsUses1979]) separately for the interference and dislike item to quantify the consistency of perceptions across personal sensing methods (e.g., Do participants who dislike one method also dislike the other methods?). <!--change to diff score comparisons?-->
...
We provided descriptive proportions of valence and themes endorsed overall for each sensing method. We then conducted six logistic regression models for each sensing method to evaluate the probability that a comment was positively and negatively valenced and the probability that the four themes were endorsed. Specifically, we conducted separate models where each theme or valence of interest was coded as 1 if present and 0 if not present. These outcomes were then regressed on the four demographic grouping variables (centered around 0) and overall and between demographic subgroup log-odds were evaluated.<!--update with final models-->
